...
export default async function Usdc() {
  const userWallet = await retrieveConnectedWalletFromCookie();
  ...
  const lastTransfers = await client
    .getContractEvents({
      abi: ERC20_ABI,
  ...
  let userBalance: string | undefined = undefined;
  if (userWallet) {
  ...
  return (
    <main className="flex min-h-screen flex-col p-24">
      <h3 className="text-xl py-4">
        USDC on Ethereum Mainnet (network is fixed on this page)
      </h3>
      <div
        className={`pb-4 ${userBalance === undefined ? "text-red-500" : ""}`}
      >
        User balance:{" "}
        {userBalance ? `${userBalance} USDC` : "Only for connected wallets"}
      </div>
      <div>
        <div className="py-4">Last transfers:</div>
        <table>
          <thead>
            <tr>
              <th>From</th>
          ...
          <tbody>
            {lastTransfers.map((transfer, i) => (
              <tr key={i} className="py-2">
                <td>{shortHex(transfer.from)}</td>
                <td>{shortHex(transfer.to)}</td>
                <td>{transfer.amount} USDC</td>
              </tr>
            ))}
          </tbody>
        </table>
